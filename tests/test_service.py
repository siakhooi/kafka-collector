...
def test_reset_duplicate_logical_name_400(client):
    client.post("/reset?name=same")
    r = client.post("/reset?name=same")
    assert r.status_code == 400
...
def test_get_files_returns_completed_captures(client):
    client.post("/reset?name=seg1")
    client.post("/reset?name=seg2")
    r = client.get("/files")
    assert r.status_code == 200
    files = r.get_json()
    assert len(files) == 2
...
def test_download_last_completed_ok(client):
    client.post("/reset?name=seg")
    r = client.get("/download")
    assert r.status_code == 200


def test_download_unknown_name_404(client):
    client.post("/reset?name=known")
